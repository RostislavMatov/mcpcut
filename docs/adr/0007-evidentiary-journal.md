...
## Поправка 2026-09-23 (к §11: раскладка `summary.md` не версионируется)

`REPORT_FORMAT_VERSION` версионирует то, что проверяется машиной: поля `report.json`, соглашение
строк `records.jsonl` и определение `chain.recomputable`. `summary.md` — **заверенная проза**: его
байты attests `summary.sha256` (A1), но `verify --report` не пересчитывает ни одной его строки, и
ни один потребитель не обязан его разбирать. Поэтому раскладка `summary.md` (разделы, колонки,
пометки) меняется **без** смены `REPORT_FORMAT_VERSION`, а контракт отчёта остаётся тем, что
проверяет `verify --report`.

Первое такое изменение — фаза 5 пула (ADR-0015, поправка 2026-09-23 фазы 5, решение владельца O1):
раздел «Pool sessions», пометка пула у заголовка дочерней сессии и колонка `server` в таблице решений.
Связка «сессия пула → дочерние» выводится из записей `kind:"pool"` (событие `attach`) того же
прохода и того же read view, что и манифест; независимо она перепроверяется по `records.jsonl`
(рецепт `jq` — в README «Pool sessions in a report»). Выгрузка `--session <сессия пула>` по-прежнему
отдаёт одну сессию: изменение смысла `scope.session` — это v2.

**Кандидат v2 (четвёртый к §12):** (d) выгрузка сессии пула вместе с её детьми и **проверяемая**
связка — поле манифеста, которое `verify --report` пересчитывает из `records.jsonl`, как сегодня
`sessionIds`. Отложено до того же интервью с аудитором, что и (a)–(c).

---

## Поправка 2026-09-23 (к §11: строки из записей вне выгрузки)

`summary.md` выгрузки одной сессии может содержать помеченные строки из записей, которых **нет** в
выгрузке: сессии пула, подключавшие эту сессию (ADR-0015, поправка 2026-09-23, EX1–EX2). Они читаются
из того же снимка, что и выгрузка, и явно помечены как не покрытые ни `records.sha256`, ни цепочкой.
Формат `report.json` — v1, `verify --report` не меняется.

## Поправка 2026-09-25 (к O3: одобрение до первого админа)

Пока на установке нет ни одного админа, `approvals approve|deny` работают без токена (решение
владельца; разбор — поправка ADR-0004 того же дня). Требование O3 «ни одной резолюции без субъекта»
сохраняется: такая резолюция несёт `actor: 'cli:_unattributed'`, а не пустое поле. Значение не
совпадает ни с одним `cli:<adminName>` (`_` вне `ADMIN_NAME_PATTERN`) и ни с досубъектной константой
`'cli'` M2–M4, поэтому отчёт отличает «из shell, до первого админа» от «из shell, до атрибуции».
`policy-edit` той же установки пишется с `adminName: null`.
