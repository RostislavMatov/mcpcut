...
import { CHILD_EXIT_GRACE_MS, DIAGNOSTIC_PREFIX } from './constants.js'
import { createReadableMessageSource } from './readable-source.js'

/**
 * The upstream half of one proxied MCP conversation: a registry record
 * turned into the `MessageSource`/`MessageSink` pair `session/core.ts`
 * consumes, for both transports. Extracted verbatim from
 * `cli/connect-upstream.ts` (M5.5, probe engine): the probe must reuse THIS
 * spawn/connect/vault-resolve path rather than grow its own, and it may not
 * live under `src/cli/**`. The CLI module re-exports everything, so existing
